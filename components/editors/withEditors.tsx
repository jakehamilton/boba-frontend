...
import { EditorActions, EditorsProvider, useEditorsDispatch, useEditorsState } from "./EditorsContext";
import { Modal, ModalWithButtons, toast } from "@bobaboard/ui-components";
import { addPostInCache, setPostTagsInCache } from "cache/post";
import { isCommentEditorState, isContributionEditorState, isEditContribution, isNewThread } from "./types";
...
const Editors = () => {
	...
	}, [setAskConfirmation, historyNavigation, setHistoryScrollPosition, setHistoryNavigation]);
	...
	const onSubmit = React.useCallback(() => {
		...
		if (navigationUrl) {
			...
			if (historyNavigation && (historyScrollPosition.x > 0 || historyScrollPosition.y > 0)) {
		...
		setNavigationUrl("");
		setHistoryNavigation(false);
	}, [onClose, setAskConfirmation, navigationUrl, router, historyScrollPosition, historyNavigation]);
	const onNavigation = React.useCallback(
		...
		[setAskConfirmation, setNavigationUrl, setHistoryScrollPosition, setHistoryNavigation]
	);
				...
				{isContributionEditorState(state) && (
					<ContributionEditorModal
						...
						onPostSaved={(post: PostType, postedBoardId: string) => {
							...
							const postedBoard = boards.find((board) => board.id == postedBoardId);
							if (!postedBoard) {
								toast.error(`The board with id ${postedBoardId} does not exist.`);
								return;
							}
...
export const withEditors = function <T>(WrappedComponent: React.FC<T>) {
	...
	ReturnedComponent.displayName = `${WrappedComponent.displayName || WrappedComponent.name}_withEditors`;
